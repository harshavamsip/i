...
class Captioner(tf.keras.Model):
    def __init__(self, tokenizer, feature_extractor, output_layer, num_layers=2, units=256, max_length=max_caption_length+1, num_heads=2, dropout_rate=0.5):
        super().__init__()
        self.tokenizer = tokenizer
        self.feature_extractor = feature_extractor
        self.output_layer = output_layer
        ...
        self.seq_embedding = SeqEmbedding(vocab_size=tokenizer.vocabulary_size(), depth=units, max_length=self.max_length)
        self.decoder_layers = [DecoderLayer(units, num_heads=num_heads, dropout_rate=dropout_rate) for _ in range(num_layers)]
    ...
    def run_and_show_attention(self, image, temperature=0.0):
        ...
        for i in range(len_result):
            ...
            plt.axis("off")
        plt.tight_layout()
        plt.suptitle(f"Predicted Caption: {result_txt}")
